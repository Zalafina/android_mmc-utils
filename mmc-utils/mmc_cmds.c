...
int send_custom_cmd(int fd, __u32 cmd, __u32 argument)
{
	int ret = 0;
	...
	if (ret){
		fprintf(stderr, "CMD%d ioctl error=%d, argument=0x%08X\n", cmd, errno, argument);
		perror("ioctl");
	}
...
int read_health(int fd, __u8 *health_info, __u32 length)
{
	int ret = 0;
	struct mmc_ioc_cmd idata;
	memset(&idata, 0, sizeof(idata));
	memset(health_info, 0, sizeof(__u8) * length);
	idata.write_flag = 0;
	...
	idata.blksz = length;
	idata.blocks = 1;
	mmc_ioc_cmd_set_data(idata, health_info);

	ret = ioctl(fd, MMC_IOC_CMD, &idata);
	if (ret){
		fprintf(stderr, "CMD8(length=%d) ioctl error=%d\n", length, errno);
		perror("CMD8 Read Health Report ioctl");
	}

	return ret;
}

int read_health_multi_cmd(int fd, __u8 *health_info)
{
	int ret = 0;
	struct mmc_ioc_multi_cmd *multi_cmd;

	memset(health_info, 0, sizeof(__u8) * HEALTH_INFO_SIZE);
	multi_cmd = calloc(1, sizeof(struct mmc_ioc_multi_cmd) + HEALTH_INFO_CMD_NUMBER * sizeof(struct mmc_ioc_cmd));
	...
	multi_cmd->num_of_cmds = HEALTH_INFO_CMD_NUMBER;

	/* health report step1 CMD60 */
	multi_cmd->cmds[0].opcode = MMC_VENDOR_CMD60;
	multi_cmd->cmds[0].arg = GET_HEALTH_REPORT_ARG_01;
	multi_cmd->cmds[0].flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
	multi_cmd->cmds[0].write_flag = 0;

	/* health report step2 CMD60 */
	multi_cmd->cmds[1].opcode = MMC_VENDOR_CMD60;
	multi_cmd->cmds[1].arg = GET_HEALTH_REPORT_ARG_02;
	multi_cmd->cmds[1].flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
	multi_cmd->cmds[1].write_flag = 0;

	/* health report step3 CMD8 */
	multi_cmd->cmds[2].opcode = MMC_SEND_EXT_CSD;
	multi_cmd->cmds[2].blksz = HEALTH_INFO_SIZE;
	...
	mmc_ioc_cmd_set_data(multi_cmd->cmds[2], health_info);

	/* send ioctl with multi-cmd */
	ret = ioctl(fd, MMC_IOC_MULTI_CMD, multi_cmd);
	if (ret){
...
int do_health_report(int nargs, char **argv)
{
	__u32 slc_reserved_blocks = 0;	/* [35:32] */
	__u32 slc_max_erase_count = 0;	/* [39:36] */
	__u32 slc_min_erase_count = 0;	/* [43:40] */
	__u32 slc_avg_erase_count = 0;	/* [47:44] */
	__u32 mlc_reserved_blocks = 0;	/* [51:48] */
	__u32 mlc_max_erase_count = 0;	/* [55:52] */
	__u32 mlc_min_erase_count = 0;	/* [59:56] */
	__u32 mlc_avg_erase_count = 0;	/* [63:60] */
	__u32 sum_reserved_blocks = 0;	/* [67:64] */
	__u8 *health_info;	
	int fd, ret;
	char *length_str;
	__u32 length = 0;
	...
	if (nargs != 3) {
		fprintf(stderr, "Usage: mmc health report <length> </path/to/mmcblkX>\n");
		exit(1);
	}

	length_str	= argv[1];
	device		= argv[2];
	length = (__u32)atoi(length_str);

	if (length > 0){
		health_info = malloc(length);
		memset(health_info, 0x00, length);
	}
	else{
		fprintf(stderr, "Heatl Report length input Error(%d)\n", length);
		exit(1);
	}
	...
	ret = send_custom_cmd(fd, MMC_VENDOR_CMD60, GET_HEALTH_REPORT_ARG_01);
	if (ret) {
		fprintf(stderr, "Get Health Report Step1 Failure of %s\n", device);
		exit(1);
	}

	ret = send_custom_cmd(fd, MMC_VENDOR_CMD60, GET_HEALTH_REPORT_ARG_02);
	if (ret) {
		fprintf(stderr, "Get Health Report Step2 Failure of %s\n", device);
		exit(1);
	}

	ret = read_health(fd, health_info, length);
	if (ret) {
		fprintf(stderr, "Could not read Health Info from %s\n", device);
		exit(1);
	}

	fprintf(stdout, "Read Health Report(%d) from %s Success.\n", length, device);

	slc_reserved_blocks = (__u32)(health_info[32] << 0) | (__u32)(health_info[33] << 8) | (__u32)(health_info[34] << 16) | (__u32)(health_info[35] << 24);
	slc_max_erase_count = (__u32)(health_info[36] << 0) | (__u32)(health_info[37] << 8) | (__u32)(health_info[38] << 16) | (__u32)(health_info[39] << 24);
	slc_min_erase_count = (__u32)(health_info[40] << 0) | (__u32)(health_info[41] << 8) | (__u32)(health_info[42] << 16) | (__u32)(health_info[43] << 24);
	slc_avg_erase_count = (__u32)(health_info[44] << 0) | (__u32)(health_info[45] << 8) | (__u32)(health_info[46] << 16) | (__u32)(health_info[47] << 24);
	mlc_reserved_blocks = (__u32)(health_info[48] << 0) | (__u32)(health_info[49] << 8) | (__u32)(health_info[50] << 16) | (__u32)(health_info[51] << 24);
	mlc_max_erase_count = (__u32)(health_info[52] << 0) | (__u32)(health_info[53] << 8) | (__u32)(health_info[54] << 16) | (__u32)(health_info[55] << 24);
	mlc_min_erase_count = (__u32)(health_info[56] << 0) | (__u32)(health_info[57] << 8) | (__u32)(health_info[58] << 16) | (__u32)(health_info[59] << 24);
	mlc_avg_erase_count = (__u32)(health_info[60] << 0) | (__u32)(health_info[61] << 8) | (__u32)(health_info[62] << 16) | (__u32)(health_info[63] << 24);
	sum_reserved_blocks = (__u32)(health_info[64] << 0) | (__u32)(health_info[65] << 8) | (__u32)(health_info[66] << 16) | (__u32)(health_info[67] << 24);

	fprintf(stdout, "Read Health Report(%d) from %s Start\n", length, device);

	fprintf(stdout, "Number of Reserved Blocks for SLC Mode = [%d]\n",		slc_reserved_blocks);
	fprintf(stdout, "Maximum Erase Count for SLC mode = [%d]\n",			slc_max_erase_count);
	fprintf(stdout, "Minimum Erase Count for SLC mode = [%d]\n",			slc_min_erase_count);
	fprintf(stdout, "Average Erase Count for SLC mode = [%d]\n",			slc_avg_erase_count);
	fprintf(stdout, "Number of Reserved Blocks for MLC mode = [%d]\n",		mlc_reserved_blocks);
	fprintf(stdout, "Maximum Erase Count for MLC mode = [%d]\n",			mlc_max_erase_count);
	fprintf(stdout, "Minimum Erase Count for MLC mode = [%d]\n",			mlc_min_erase_count);
	fprintf(stdout, "Average Erase Count for MLC mode = [%d]\n",			mlc_avg_erase_count);
	fprintf(stdout, "Number of Reserved Blocks for SLC+MLC mode = [%d]\n",	sum_reserved_blocks);

	fprintf(stdout, "Read Health Report(%d) from %s End\n", length, device);
...
{
	__u8 health_info[HEALTH_INFO_SIZE];
	__u32 slc_reserved_blocks = 0;	/* [35:32] */
	__u32 slc_max_erase_count = 0;	/* [39:36] */
	__u32 slc_min_erase_count = 0;	/* [43:40] */
	__u32 slc_avg_erase_count = 0;	/* [47:44] */
	...
	if (nargs != 2) {
		fprintf(stderr, "Usage: mmc health read </path/to/mmcblkX>\n");
		exit(1);
	}
	...
	ret = read_health(fd, health_info, HEALTH_INFO_SIZE);
	if (ret) {
		fprintf(stderr, "Could not read Health Info from %s\n", device);
		exit(1);
	}
...
int do_read_health_multicmd(int nargs, char **argv)
{
	__u8 health_info[HEALTH_INFO_SIZE];
	int fd, ret;
	char *device;
	...
	if (nargs != 2) {
		fprintf(stderr, "Usage: mmc health read multi </path/to/mmcblkX>\n");
		exit(1);
	}
	...
	ret = read_health_multi_cmd(fd, health_info);
	if (ret) {
		fprintf(stderr, "Could not read Health Info(Multi CMD) from %s\n", device);
		exit(1);
	}
